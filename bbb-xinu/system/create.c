	...
	)
{
	...
	//Stack consumption code starts
	int cnt=1;
	char * startAddr = (char *)saddr;
	kprintf("Allocated Stack of size %d bytes \n",ssize);

	while(1)
	{
		if(cnt == ssize)
		{
			//kprintf("addr: %u, cnt:%d\n",startAddr, cnt);
			kprintf("Intialized %d bytes with dummy values \n",cnt);
			break;
		}
